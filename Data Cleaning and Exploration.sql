...
select Distinct MONTH(Order_date) as month_ , YEAR(Order_date) as year_
from dbo.Sales_products
-- data represent all months in 2019 and january in 2020
...
-- the best year for sale is [2019] with (34456867.65) but
   -- data represent all months in 2019 and only january in 2020

   -- let's see sales in january in 2019 ,2020
select YEAR(Order_date) as sales_year, MONTH(Order_date)as sales_month,
      ROUND( sum(Total_sales),2) as total_sales
from dbo.Sales_products
where MONTH(Order_date) = 1
group by YEAR(Order_date),MONTH(Order_date)
-- there is regression in sales in 2020
...
select a.Product as A , b.Product as B, Count(*) as frequency  into pro_sold_together
from order_items a
join order_items b
